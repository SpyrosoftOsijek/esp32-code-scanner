...
esp_err_t ll_cam_set_pin(cam_obj_t *cam, const camera_config_t *config)
{
    ...
    ESP_LOGE(TAG, "gpio_set_direction(config->pin_vsync, GPIO_MODE_INPUT)");
    gpio_set_pull_mode(config->pin_vsync, GPIO_FLOATING);
    ESP_LOGE(TAG, "gpio_set_pull_mode(config->pin_vsync, GPIO_FLOATING)");
    ...
    ESP_LOGE(TAG, "gpio_set_direction(config->pin_href, GPIO_MODE_INPUT)");
    gpio_set_pull_mode(config->pin_href, GPIO_FLOATING);
    ESP_LOGE(TAG, "gpio_set_pull_mode(config->pin_href, GPIO_FLOATING)");
    ...
    for (int i = 0; i < 8; i++) {
        ...
        ESP_LOGE(TAG, "gpio_set_direction(data_pins[i], GPIO_MODE_INPUT)");
        gpio_set_pull_mode(data_pins[i], GPIO_FLOATING);
        ESP_LOGE(TAG, "gpio_set_pull_mode(data_pins[i], GPIO_FLOATING)");
    ...
    if (config->pin_xclk >= 0) { 
        ...
        ESP_LOGE(TAG, "gpio_set_direction(config->pin_xclk, GPIO_MODE_OUTPUT)");
        gpio_set_pull_mode(config->pin_xclk, GPIO_FLOATING);
        ESP_LOGE(TAG, "gpio_set_pull_mode(config->pin_xclk, GPIO_FLOATING)");
